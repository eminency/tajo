...
#ifndef TAJO_UTIL_MEMORY_H_
#define TAJO_UTIL_MEMORY_H_
...
// TODO - to consider other architectures
const uint32_t X86_64_SIMD_ALIGNED_SIZE = 16;

size_t ComputeAlignedSize(const size_t size);
...
class BufferAllocator {
 ...
 private:
  ...
  void operator=(const BufferAllocator &);
};
...
class HeapBufferAllocator : public BufferAllocator {
 ...
 private:
  ...
  HeapBufferAllocator(const HeapBufferAllocator &);

  void operator=(const HeapBufferAllocator &);
};
...
class ClearingBufferAllocator : public BufferAllocator {
 ...
 private:
  ...
  ClearingBufferAllocator(const ClearingBufferAllocator &);

  void operator=(const ClearingBufferAllocator &);
};

class MutexLockMaybe {
 public:
  explicit MutexLockMaybe(boost::mutex *mu) :
      mu_(mu) {
 ...
 private:
  boost::mutex *const mu_;

  MutexLockMaybe(const MutexLockMaybe &);

  void operator=(const MutexLockMaybe &);
};
...
class StaticQuota : public Quota<thread_safe> {
 ...
 private:
  ...
  StaticQuota(const StaticQuota &);

  void operator=(const StaticQuota &);
};
...
class MemoryStatisticsCollectorInterface {
 ...
 private:
  MemoryStatisticsCollectorInterface(const MemoryStatisticsCollectorInterface &);

  void operator=(const MemoryStatisticsCollectorInterface &);
};
...
class MemoryStatisticsCollectingBufferAllocator : public BufferAllocator {
 ...
 private:
  ...
  BufferAllocator *delegate_;
  boost::scoped_ptr<MemoryStatisticsCollectorInterface>
...
class ThreadSafeBufferAllocator : public BufferAllocator {
 ...
 private:
  ...
  ThreadSafeBufferAllocator(const ThreadSafeBufferAllocator &);

  void operator=(const ThreadSafeBufferAllocator &);
};
...
class OwningThreadSafeBufferAllocator
 ...
 private:
  boost::scoped_ptr<DelegateAllocatorType> delegate_owned_;
};
...
class GuaranteeMemory : public BufferAllocator {
 ...
 private:
  ...
  GuaranteeMemory(const GuaranteeMemory &);

  void operator=(const GuaranteeMemory &);
};
